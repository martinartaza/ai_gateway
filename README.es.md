[![Inglés](https://img.shields.io/badge/lang-english-blue)](README.md)
...
sequenceDiagram
    ...
    participant M as Modelo LLM<br/>(Qwen 2.5 3B)
...
1. Detener el container `web` para no recibir requests durante el cambio
2. Borrar el modelo anterior
3. Descargar el nuevo modelo
4. Actualizar `.env`
5. Levantar todo
...
# 2. Borrar el modelo anterior
docker compose exec ollama ollama rm MODELO_ANTERIOR

# 3. Descargar el nuevo modelo (~2 GB, puede tardar)
docker compose exec ollama ollama pull NUEVO_MODELO

# 4. Actualizar .env
LLM_MODEL_NAME=NUEVO_MODELO
...
|--------|------------------------|-----------|------------|
| Qwen 2.5 0.5B | `qwen2.5:0.5b` | ~600 MB | Extracción, clasificación simple |
| Qwen 2.5 1.5B | `qwen2.5:1.5b` | ~1.5 GB | Mejor comprensión, misma velocidad |
| Qwen 2.5 3B *(actual)* | `qwen2.5:3b` | ~2.5 GB | Respuestas más elaboradas, mejor razonamiento |
...
| `LLM_MODEL_NAME` | Nombre del modelo en Ollama | `qwen2.5:3b` |
| `LLM_NUM_CTX` | Ventana de contexto en tokens (system + historial + prompt). Qwen 2.5 3B soporta hasta 32768. | `8192` |
| `LLM_TIMEOUT` | Timeout en segundos para la llamada al LLM. Aumentar para modelos lentos o contextos grandes. | `120` |
